...
```{r estrut, echo=FALSE}
cat(system("cd prrpac && stat -c '%A %n' *", intern = TRUE),
    sep = "\n")
...
```{r escreve, echo=FALSE, comment=NA, cache=TRUE}
write("Imports: lattice",
      file = "prrpac/DESCRIPTION", append = TRUE)
cat(system("cat prrpac/DESCRIPTION", intern = TRUE), sep = "\n")
```
...
build("prrpac/")
```
```{r buildtrue2, echo=FALSE, eval=TRUE, cache=TRUE}
## Para mostrar o output da chamada da funcao foi necessario chamar o
## Rscript dentro do system
cat(system("Rscript -e 'build(\"prrpac/\")'", intern = TRUE),
    sep = "\n")
```
