...
```{r cpdescr, include=FALSE}
file.copy("aux/DESCRIPTION", "prrpac/DESCRIPTION", overwrite = TRUE)
```
```{r catdescr, echo=FALSE}
cat(system("cat prrpac/DESCRIPTION", intern = TRUE), sep = "\n")
```
...
```{r cpsoma, echo=FALSE, include=FALSE}
file.create("prrpac/R/soma.R")
file.copy("aux/soma.R", "prrpac/R/soma.R", overwrite = TRUE)
```
```{r catsoma2, echo=FALSE}
cat(system("cat prrpac/R/soma.R", intern = TRUE), sep = "\n")
```
...
document()
```
```{r doctrue, echo=FALSE, eval=TRUE, dependson="cpsoma", cache=TRUE}
document("prrpac/")
## Armazena o NAMESPACE atual para mostrar depois
namespace2 <- readLines("prrpac/NAMESPACE")
```
...
```{r catsomard, echo=FALSE}
cat(system("cat prrpac/man/soma.Rd", intern = TRUE), sep = "\n")
```
...
cat(system("head -2 prrpac/man/soma.Rd", intern = TRUE), sep = "\n")
```
...
escrita de maneira correta, é utilizando a função `check_man()` do
pacote `devtools`:

```{r checkdocfalse, echo=TRUE, eval=FALSE}
check_man()
```
```{r checkdoctrue, echo=FALSE, eval=TRUE, cache=TRUE}
check_man("prrpac/")
```
...
* `@export`: esta tag é a responsável por sua função ficar disponível
  ...
  NAMESPACE abaixo). Se você não quer que a
...
```{r catnam, echo=FALSE, comment=NA, cache=TRUE}
cat(namespace, sep = "\n")
```
...
```{r catnam1, echo=FALSE, comment=NA, cache=TRUE}
cat(namespace2, sep = "\n")
```
...
Já vimos anteriromemte na seção DESCRIPTION que existe um campo
...
file.create("prrpac/R/meuxy.R")
file.copy("aux/meuxy.R", "prrpac/R/meuxy.R", overwrite = TRUE)
```
```{r echo=FALSE, comment=NA, cache=TRUE}
cat(system("cat prrpac/R/meuxy.R", intern = TRUE), sep = "\n")
```
...
```{r catnam2, echo=FALSE, comment=NA}
cat(system("cat prrpac/NAMESPACE", intern = TRUE), sep = "\n")
```
...
```{r catnamtail, echo=FALSE, comment=NA}
cat(system("tail -1 prrpac/NAMESPACE", intern = TRUE), sep = "\n")
```

Note que, neste exemplo, utilizamos a tag

```{r mimport, eval=FALSE}
...
inserção deve ser feita manualmente, ou utilizando a função
`use_package()` do `devtools`. Por exemplo, o comando abaixo irá inserir
automaticamente o pacote `lattice` no campo `Imports` do arquivo
`DESCRIPTION`.

```{r usefalse1, echo=TRUE, eval=FALSE}
use_package("lattice")
```
```{r usetrue1, echo=FALSE, eval=TRUE, dependson="meuxy", cache=TRUE}
use_package("lattice", pkg = "prrpac/")
```

Dessa forma, o arquivo `DESCRIPTION` ficaria atualizado com o seguinte
conteúdo (repare a última linha):

```{r catdescr1, echo=FALSE, comment=NA, cache=TRUE}
cat(system("cat prrpac/DESCRIPTION", intern = TRUE), sep = "\n")
```
...
```{r checkfalse, echo=TRUE, eval=FALSE}
check()
```
```{r checktrue1, echo=FALSE, eval=TRUE, cache=TRUE}
## Essa chamada so mostra a parte inicial do output
check("prrpac/")
```
```{r checktrue2, echo=FALSE, eval=TRUE, cache=TRUE, collapse=TRUE}
## Para mostrar o output da chamada da funcao foi necessario chamar o
## Rscript dentro do system
cat(system("Rscript -e 'devtools::check(\"prrpac/\")'", intern = TRUE),
    sep = "\n")
```
...
```{r buildfalse, echo=TRUE, eval=FALSE}
build()
```
```{r buildtrue1, echo=FALSE, eval=TRUE, cache=TRUE, results='hide'}
## Essa chamada so mostra a parte inicial do output
build("prrpac/")
```
```{r buildtrue2, echo=FALSE, eval=TRUE, cache=TRUE, collapse=TRUE}
## Para mostrar o output da chamada da funcao foi necessario chamar o
## Rscript dentro do system
cat(system("Rscript -e 'devtools::build(\"prrpac/\")'", intern = TRUE),
    sep = "\n")
```
